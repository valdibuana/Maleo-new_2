...
router.get("/", auth_1.verifyJWT, async (req, res) => {
    try {
        ...
        let config = await prisma_1.prisma.gradeConfig.findUnique({
            where: {
        ...
        if (!config) {
            config = await prisma_1.prisma.gradeConfig.create({
                data: {
                    teacherId: Number(teacherId),
                    subjectId: Number(subjectId),
                    classId: Number(classId),
...
router.get("/summary", auth_1.verifyJWT, async (req, res) => {
    try {
        ...
        const summary = students.map(student => {
            ...
            const getGradeLabel = (score) => {
                if (score >= 90)
                    return { letter: "A", label: "Istimewa" };
                if (score >= 80)
                    return { letter: "B", label: "Baik" };
                if (score >= 70)
                    return { letter: "C", label: "Cukup" };
                if (score >= 60)
                    return { letter: "D", label: "Perlu Usaha" };
...
router.put("/:id", auth_1.verifyJWT, (0, role_1.checkRole)("admin", "teacher"), async (req, res) => {
    try {
        ...
        const hasNegative = components.some((c) => Number(c.weight) < 0);
        if (hasNegative) {
            res.status(400).json({
                success: false,
        ...
        const updated = await prisma_1.prisma.gradeConfig.findUnique({
            where: { id: configId },
            include: {
                components: { orderBy: { order: "asc" } }
            }
...
//# sourceMappingURL=grade-config.route.js.map