...
const buildDateFilter = (query) => {
    ...
    if (startDate && endDate) {
        ...
        if (diffDays > 366)
            return { error: "Rentang tanggal maksimal 366 hari" };
...
router.get("/summary", auth_1.verifyJWT, async (req, res) => {
    try {
        ...
        const data = Array.from(summaryMap.values()).sort((a, b) => a.studentName.localeCompare(b.studentName));
        // Compute date range labels for UI
        const dateLabel = buildDateLabel(req.query);
...
const buildDateLabel = (query) => {
    ...
    if (date)
        return fmt(new Date(String(date)));
    return "Semua Data";
};
...
router.get("/export/excel", auth_1.verifyJWT, async (req, res) => {
    try {
        ...
        if (month && year) {
            ...
            const fmtBlock = (stats) => `H:${stats.hadir} S:${stats.sakit} I:${stats.izin} A:${stats.alpa}`;
