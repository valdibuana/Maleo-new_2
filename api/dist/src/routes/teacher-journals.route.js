...
router.get('/', auth_1.verifyJWT, async (req, res) => {
    try {
        ...
        const [total, journals] = await prisma_1.prisma.$transaction([
            prisma_1.prisma.teacherJournal.count({ where }),
            prisma_1.prisma.teacherJournal.findMany({
                where,
                include: {
...
router.get('/:id', auth_1.verifyJWT, async (req, res) => {
    try {
        const { id } = req.params;
        const journal = await prisma_1.prisma.teacherJournal.findUnique({
            where: { id: Number(id) },
            include: {
                teacher: { select: { id: true, name: true, nip: true } },
                class: { select: { id: true, name: true, level: true } },
...
router.post('/', auth_1.verifyJWT, async (req, res) => {
    try {
        ...
        const journal = await prisma_1.prisma.teacherJournal.create({
            data: {
                date: new Date(date),
                material,
                objective,
...
router.put('/:id', auth_1.verifyJWT, async (req, res) => {
    try {
        const { id } = req.params;
        const existing = await prisma_1.prisma.teacherJournal.findUnique({ where: { id: Number(id) } });
        if (!existing) {
            return res.status(404).json({ success: false, message: 'Journal not found' });
        }
        // Access check for teacher
        ...
        if (activityNotes !== undefined)
            updateData.activityNotes = activityNotes;
        if (constraints !== undefined)
            updateData.constraints = constraints;
        if (followUp !== undefined)
            updateData.followUp = followUp;
        const journal = await prisma_1.prisma.teacherJournal.update({
            where: { id: Number(id) },
            data: updateData
        });
...
router.delete('/:id', auth_1.verifyJWT, async (req, res) => {
    try {
        ...
        await prisma_1.prisma.teacherJournal.delete({
            where: { id: Number(id) }
        });
...
//# sourceMappingURL=teacher-journals.route.js.map