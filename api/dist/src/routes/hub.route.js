    ...
    try {
        ...
        if (user.role === "teacher") {
            if (user.teacherId) {
                ...
                if (!teacher)
                    return res.status(403).json({ success: false, message: "Profil guru tidak ditemukan atau telah dihapus." });
            ...
            if (user.studentId) {
                const student = await prisma_1.prisma.student.findUnique({ where: { id: user.studentId } });
                if (!student)
                    return res.status(403).json({ success: false, message: "Profil siswa tidak ditemukan atau telah dihapus." });
    ...
    try {
        ...
        if (role === "teacher") {
            ...
            const [subjectCount, assignmentCount, daySchedules, grades] = await Promise.all([
                prisma_1.prisma.subject.count({ where: { teacherId } }),
                prisma_1.prisma.assignment.count({ where: { teacherId, dueDate: { gte: today } } }),
            ...
            const [assignmentCount, daySchedules, grades, attendances] = await Promise.all([
                prisma_1.prisma.assignment.count({ where: { classId, dueDate: { gte: today } } }),
                prisma_1.prisma.schedule.findMany({
                ...
                prisma_1.prisma.attendance.findMany({ where: { studentId, date: { gte: new Date(today.getFullYear(), today.getMonth(), 1) } } })
            ]);
        ...
        res.json({
            ...
            data: {
                stats,
                todaySchedules: schedules.map((s) => ({
...
router.get("/announcements", async (req, res) => {
    try {
        ...
        if (role === "student") {
            ...
            targets = [
                client_1.AnnouncementTarget.all,
                client_1.AnnouncementTarget.student,
                client_1.AnnouncementTarget.teacher,
                client_1.AnnouncementTarget.guardian,
            ];
        }
...
router.get("/assignments", async (req, res) => {
    try {
        ...
        if (role === "student" && req.studentId) {
            const subs = await prisma_1.prisma.assignmentSubmission.findMany({
        ...
        const result = assignments.map((a) => ({
            ...
            studentSubmission: role === "student" ? (studentSubmissions.has(a.id)
...
router.post("/assignments/upload", multer_1.uploadMaterial.single("file"), async (req, res) => {
    try {
        if (req.user.role !== "teacher") {
            return res.status(403).json({ success: false, message: "Akses ditolak." });
        }
        ...
        const fileType = path_1.default.extname(req.file.originalname).replace(".", "").toLowerCase();
...
router.get("/assignments/:id", async (req, res) => {
    try {
        ...
        if (role === "student" && req.studentId) {
            const sub = await prisma_1.prisma.assignmentSubmission.findUnique({
        ...
        if (role === "teacher") {
            submissions = await prisma_1.prisma.assignmentSubmission.findMany({
                where: { assignmentId },
...
router.post("/assignments/:id/submit", multer_1.uploadMaterial.single("file"), async (req, res) => {
    try {
        ...
        const existing = await prisma_1.prisma.assignmentSubmission.findUnique({
        ...
        if (req.file) {
            ...
            fileType = path_1.default.extname(req.file.originalname).replace(".", "").toLowerCase();
...
router.put("/assignments/submit/:submissionId", multer_1.uploadMaterial.single("file"), async (req, res) => {
    try {
        if (req.user.role !== "student") {
        ...
        const studentId = req.studentId;
        const existing = await prisma_1.prisma.assignmentSubmission.findFirst({
        ...
        const { content } = req.body;
        const updated = await prisma_1.prisma.assignmentSubmission.update({
...
router.delete("/assignments/submit/:submissionId", async (req, res) => {
    try {
        if (req.user.role !== "student") {
        ...
        await prisma_1.prisma.assignmentSubmission.delete({ where: { id: submissionId } });
...
router.get("/teacher-classes", async (req, res) => {
    try {
        ...
        const subjectLevelClasses = await prisma_1.prisma.class.findMany({
...
router.get("/grades", async (req, res) => {
    try {
        ...
        let grades = [];
        if (role === "student") {
            const where = { studentId: req.studentId };
            if (subjectId)
                where.subjectId = Number(subjectId);
            if (type)
                where.type = type;
            grades = await prisma_1.prisma.grade.findMany({
                where,
                include: {
                    subject: { select: { name: true } }
                },
            ...
            const where = { subject: { teacherId: req.teacherId } };
            if (subjectId)
                where.subjectId = Number(subjectId);
            if (classId)
...
router.delete("/grades/:id", async (req, res) => {
    try {
        ...
        const existing = await prisma_1.prisma.grade.findFirst({
        ...
        await prisma_1.prisma.grade.delete({ where: { id: gradeId } });
...
router.post("/grades/:id/lock", async (req, res) => {
    try {
        if (req.user.role !== "teacher") {
            return res.status(403).json({ success: false, message: "Akses ditolak." });
        }
...
//# sourceMappingURL=hub.route.js.map