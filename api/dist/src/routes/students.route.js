...
router.post("/import", auth_1.verifyJWT, (0, role_1.checkRole)("admin"), upload.single("file"), async (req, res) => {
    try {
        if (!req.file) {
        ...
        const errors = [];
        const warnings = [];
        const allClasses = await prisma_1.prisma.class.findMany();
        ...
        for (let i = 1; i < data.length; i++) {
            ...
            const existingStudent = await prisma_1.prisma.student.findUnique({ where: { nis } });
            try {
                await prisma_1.prisma.$transaction(async (tx) => {
                    if (existingStudent) {
                        ...
                        const hashedPassword = await bcryptjs_1.default.hash(userCode, salt);
...
router.get("/export", auth_1.verifyJWT, (0, role_1.checkRole)("admin"), async (req, res) => {
    try {
...
router.get("/", auth_1.verifyJWT, async (req, res) => {
    try {
        ...
        if (role === roles_1.ROLES.STUDENT) {
            ...
            const student = await prisma_1.prisma.student.findFirst({ where: { user: { id: userId }, deletedAt: null } });
            if (!student)
        ...
        const result = students.map((s) => ({
            ...
            status: s.status,
            userCode: isAdminOrStaff ? (s.user?.userCode || null) : undefined,
            guardians: isAdminOrStaff ? s.guardians.map((g) => ({ id: g.id, name: g.name })) : undefined,
        }));
...
router.post("/", auth_1.verifyJWT, (0, role_1.checkRole)("admin"), (0, validate_1.validate)(studentSchema), async (req, res) => {
    try {
        ...
        let isUnique = false;
        while (!isUnique) {
            const rawCode = await (0, userCode_1.generateUniqueUserCode)("student");
            userCode = `S${rawCode}`;
