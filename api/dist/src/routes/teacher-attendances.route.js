...
router.post("/checkin", auth_1.verifyJWT, async (req, res) => {
    try {
        ...
        const isLate = status === 'hadir' && (hour > CUTOFF_HOUR ||
...
router.get("/export", auth_1.verifyJWT, async (req, res) => {
    try {
        ...
        if (q.startDate && q.endDate)
            fileNameDate = `${q.startDate}_sampai_${q.endDate}`;
        else if (q.weekStart)
            fileNameDate = `Minggu_dari_${q.weekStart}`;
        else if (q.date)
            fileNameDate = `Tanggal_${q.date}`;
        else if (q.month && q.year)
            fileNameDate = `${new Date(Number(q.year), Number(q.month) - 1).toLocaleString('id-ID', { month: 'long' })}_${q.year}`;
