...
// ──────────────────────────────────────────────
const atpOwnershipGuard = async (req, res, next) => {
    try {
        const atpId = Number(req.params.atpId || req.params.id);
        if (!atpId)
            return next(); // lewati jika tidak ada atpId di param
        const atp = await prisma_1.prisma.aTP.findUnique({ where: { id: atpId } });
        if (!atp) {
...
// ──────────────────────────────────────────────
const uploadDir = path_1.default.join(process.cwd(), "uploads", "atp-materials");
...
async function generateNomorInduk(subjectCode, year) {
    const count = await prisma_1.prisma.aTP.count();
...
router.get("/student/my-atp", async (req, res) => {
    try {
        ...
        const atpList = await prisma_1.prisma.aTP.findMany({
            where: {
...
router.post("/student/materials/:materialId/access", async (req, res) => {
    try {
        ...
        await prisma_1.prisma.aTPMaterialAccess.upsert({
...
router.post("/generate", teacherGuard, async (req, res) => {
    try {
        ...
        const [subjectData, classData] = await Promise.all([
            prisma_1.prisma.subject.findUnique({ where: { id: Number(subjectId) }, select: { gradeLevel: true, name: true } }),
            prisma_1.prisma.class.findUnique({ where: { id: Number(classId) }, select: { level: true, name: true } }),
        ]);
        if (!subjectData) {
        ...
        // Cek duplikat ATP
        const existing = await prisma_1.prisma.aTP.findFirst({
            where: {
                subjectId: Number(subjectId),
        ...
        const atp = await prisma_1.prisma.$transaction(async (tx) => {
        ...
        const atpWithMeetings = await prisma_1.prisma.aTP.findUnique({
            where: { id: atp.id },
            include: {
...
router.get("/:id", teacherGuard, atpOwnershipGuard, async (req, res) => {
    try {
        const atp = await prisma_1.prisma.aTP.findUnique({
...
router.put("/:id", teacherGuard, atpOwnershipGuard, async (req, res) => {
    try {
        const { learningObjective, learningStrategy, teacherNote } = req.body;
        const updated = await prisma_1.prisma.aTP.update({
            where: { id: Number(req.params.id) },
...
router.put("/:id/publish", teacherGuard, atpOwnershipGuard, async (req, res) => {
    try {
        const atpId = Number(req.params.id);
        const now = new Date();
        await prisma_1.prisma.$transaction([
            prisma_1.prisma.aTP.update({
                where: { id: atpId },
                data: { status: "published", publishedAt: now },
            }),
            prisma_1.prisma.aTPMeeting.updateMany({
                where: { atpId },
                data: { status: "published", publishedAt: now },
            }),
        ]);
...
router.put("/:id/archive", teacherGuard, atpOwnershipGuard, async (req, res) => {
    try {
        await prisma_1.prisma.aTP.update({
            where: { id: Number(req.params.id) },
...
router.delete("/:id", teacherGuard, atpOwnershipGuard, async (req, res) => {
    try {
        await prisma_1.prisma.aTP.delete({ where: { id: Number(req.params.id) } });
...
router.get("/:id/bundle", teacherGuard, atpOwnershipGuard, async (req, res) => {
    try {
        const atp = await prisma_1.prisma.aTP.findUnique({
            where: { id: Number(req.params.id) },
            include: {
...
router.put("/:atpId/meetings/:meetingId", teacherGuard, atpOwnershipGuard, async (req, res) => {
    try {
        const meetingId = Number(req.params.meetingId);
        const atpId = Number(req.params.atpId);
        // Pastikan meeting ada di ATP ini
        const meeting = await prisma_1.prisma.aTPMeeting.findFirst({
        ...
        const updated = await prisma_1.prisma.aTPMeeting.update({
            where: { id: meetingId },
...
router.put("/:atpId/meetings/:meetingId/publish", teacherGuard, atpOwnershipGuard, async (req, res) => {
    try {
        const meetingId = Number(req.params.meetingId);
        const atpId = Number(req.params.atpId);
        const meeting = await prisma_1.prisma.aTPMeeting.findFirst({ where: { id: meetingId, atpId } });
        if (!meeting) {
            return res.status(404).json({ success: false, message: "Pertemuan tidak ditemukan." });
        }
        const updated = await prisma_1.prisma.aTPMeeting.update({
...
router.post("/:atpId/meetings/:meetingId/materials/upload", teacherGuard, atpOwnershipGuard, upload.single("file"), async (req, res) => {
    try {
        if (!req.file) {
        ...
        const material = await prisma_1.prisma.aTPMaterial.create({
            data: {
                meetingId,
...
router.post("/:atpId/meetings/:meetingId/materials/link", teacherGuard, atpOwnershipGuard, async (req, res) => {
    try {
...
router.delete("/:atpId/meetings/:meetingId/materials/:materialId", teacherGuard, atpOwnershipGuard, async (req, res) => {
    try {
        const materialId = Number(req.params.materialId);
        const material = await prisma_1.prisma.aTPMaterial.findUnique({ where: { id: materialId } });
        if (!material) {
        ...
        await prisma_1.prisma.aTPMaterial.delete({ where: { id: materialId } });
...
router.post("/:id/import-excel", teacherGuard, atpOwnershipGuard, upload.single("excel"), async (req, res) => {
    try {
        if (!req.file) {
        ...
        for (const row of rows) {
            ...
            if (!meetingNumber || meetingNumber < 1)
                continue;
            const meeting = await prisma_1.prisma.aTPMeeting.findFirst({ where: { atpId, meetingNumber } });
            if (meeting) {
                await prisma_1.prisma.aTPMeeting.update({
                    where: { id: meeting.id },
                    data: {
...
//# sourceMappingURL=atp.route.js.map