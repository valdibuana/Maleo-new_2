 ...
 */
(0, vitest_1.describe)('Task 10: Integration Testing - Complete Material Access Flows', () => {
    let testData;
    (0, vitest_1.beforeEach)(async () => {
        await (0, setup_1.cleanupTestData)();
        // Setup test data
        const { student: student1 } = await (0, setup_1.createTestStudent)();
        const { student: student2 } = await (0, setup_1.createTestStudent)();
        const { material: material1 } = await (0, setup_1.createTestMaterial)();
    ...
    (0, vitest_1.it)('[INTEGRATION] Complete student material access flow', async () => {
        /**
        ...
        const trackingRecords = await setup_1.prisma.studentMaterialAccess.findMany({
            where: { studentId, materialId },
        });
        (0, vitest_1.expect)(trackingRecords.length).toBe(1);
        ...
        (0, vitest_1.expect)(secondAccess.id).toBe(firstAccess.id); // Same record
        (0, vitest_1.expect)(secondAccess.accessedAt.getTime()).toBeGreaterThan(firstAccess.accessedAt.getTime());
        console.log('   ✅ Repeat access updated timestamp (no crash)');
        console.log('   ✅ Complete flow PASSED\n');
    });
    (0, vitest_1.it)('[INTEGRATION] Multiple students accessing same material', async () => {
        /**
        ...
        // Student 1 accesses
        await setup_1.prisma.studentMaterialAccess.upsert({
    ...
    (0, vitest_1.it)('[INTEGRATION] Same student accessing multiple materials', async () => {
        /**
    ...
    (0, vitest_1.it)('[INTEGRATION] Rapid repeat access (race condition simulation)', async () => {
        /**
        ...
        const rapidAccess = await Promise.allSettled([
            setup_1.prisma.studentMaterialAccess.upsert({
                where: { studentId_materialId: { studentId, materialId } },
                create: { studentId, materialId },
                update: { accessedAt: new Date() },
            }),
        ...
        (0, vitest_1.expect)(successes.length).toBe(3); // All upserts succeed
        console.log('   ✅ All 3 concurrent upserts succeeded');
        // Verify still only one record
        const finalRecords = await setup_1.prisma.studentMaterialAccess.findMany({
            where: { studentId, materialId },
        });
        (0, vitest_1.expect)(finalRecords.length).toBe(1);
        console.log('   ✅ Still only 1 record (no race condition duplicates)');
        console.log('   ✅ Race condition test PASSED\n');
    });
    (0, vitest_1.it)('[INTEGRATION] Teacher analytics after multiple accesses', async () => {
        /**
        ...
        const materialWithAnalytics = await setup_1.prisma.sessionMaterial.findUnique({
            where: { id: materialId },
            include: {
                _count: {
                    select: { access: true },
        ...
        (0, vitest_1.expect)(materialWithAnalytics?._count.access).toBe(2); // 2 unique students
        (0, vitest_1.expect)(materialWithAnalytics?.access.length).toBe(2);
    ...
    (0, vitest_1.it)('[INTEGRATION] Authorization checks enforced', async () => {
        /**
        ...
        const studentAccess = await setup_1.prisma.studentMaterialAccess.upsert({
            where: {
    ...
    (0, vitest_1.it)('[INTEGRATION] Database state after complex session', async () => {
        /**
        ...
        const totalRecords = await setup_1.prisma.studentMaterialAccess.count();
        (0, vitest_1.expect)(totalRecords).toBe(5); // 3 + 2 = 5 unique combinations
        console.log('   ✅ Total records: 5 (correct)');
        // Verify per-material counts
        const material1Count = await setup_1.prisma.studentMaterialAccess.count({
            where: { materialId: materials[0] },
        });
        (0, vitest_1.expect)(material1Count).toBe(2); // Both students
        const material2Count = await setup_1.prisma.studentMaterialAccess.count({
            where: { materialId: materials[1] },
        });
        (0, vitest_1.expect)(material2Count).toBe(2); // Both students
        const material3Count = await setup_1.prisma.studentMaterialAccess.count({
            where: { materialId: materials[2] },
        });
        (0, vitest_1.expect)(material3Count).toBe(1); // Only student 1
...
//# sourceMappingURL=lms-tracking-integration.test.js.map