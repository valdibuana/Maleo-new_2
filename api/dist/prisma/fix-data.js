...
async function fixData() {
    ...
    for (const t of teachers) {
        if (!/^\d{4}$/.test(t.nip)) {
            ...
            if (newNip.length > 4)
                newNip = newNip.substring(0, 4);
            else if (newNip.length > 0 && newNip.length < 4)
                newNip = newNip.padStart(4, '0');
            if (t.nip === '000000000')
                newNip = '0000';
    ...
    for (const u of users) {
        if (u.userCode && /^\d{3}$/.test(u.userCode)) {
            ...
            if (u.role === 'teacher')
                ...
                prefix = 'O';
            else if (u.role === 'kepala_sekolah')
                prefix = 'K';
            if (prefix) {
                const newCode = prefix + u.userCode;
                let updateData = { userCode: newCode };
                ...
                if (u.force_change_password) {
                    ...
                    if (defPass) {
                        updateData.password = await bcryptjs_1.default.hash(defPass, 10);
                    }
                }
