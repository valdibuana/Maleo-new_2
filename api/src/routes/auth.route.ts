...
const forgotPasswordSchema = z.object({
  identifier: z.string().min(1, "Email / NIS / NIP wajib diisi"),
  defaultPassword: z.string().min(1, "Password default sistem wajib diisi"),
  newPassword: z
    .string()
    .min(8, "Password minimal 8 karakter")
    .regex(/[A-Z]/, "Password harus mengandung minimal 1 huruf besar")
...
router.post(
  ...
  async (req, res: Response) => {
    try {
      const { identifier, defaultPassword, newPassword } =
        req.body as z.infer<typeof forgotPasswordSchema>;
