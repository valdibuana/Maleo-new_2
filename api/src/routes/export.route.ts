 ...
 */
router.get('/attendance/student', verifyJWT, checkRole('admin', 'kepala_sekolah'), async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Generate Excel
    const buffer = await AttendanceExportService.generateStudentAttendanceExcel(
      classData.name,
      startDate as string,
      endDate as string,
      data
    ...
    res.status(500).json({ 
      success: false, 
      message: 'Gagal generate Excel: ' + (error as Error).message 
    });
  }
});
 ...
 */
router.get('/attendance/teacher', verifyJWT, checkRole('admin', 'kepala_sekolah'), async (req: AuthRequest, res: Response) => {
  try {
    ...
    const monthIndex = parseInt(month as string) - 1; // 0-based (0 = January)
    ...
    // Generate Excel
    const buffer = await AttendanceExportService.generateTeacherAttendanceExcel(
