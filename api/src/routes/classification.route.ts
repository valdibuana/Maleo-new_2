...
router.get(
  "/student/:studentId",
  verifyJWT,
  checkRole("admin", "teacher", "student", "guardian"),
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      let classification = await prisma.studentClassification.findUnique({
        where: {
      ...
      const subjectGrades: Record<number, { name: string, types: Record<string, { total: number, count: number }>, avg: number }> = {};
      ...
      classification = await prisma.studentClassification.create({
        data: {
          studentId,
          semester: semesterNum,
          academicYearId: activeYear.id,
