...
import {
  BookOpen, Loader2, Search, Calendar, User,
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      <PageHeader
        ...
        subtitle="Pantau aktivitas pembelajaran seluruh guru di sekolah."
      />
      ...
      <Card className="p-4">
        <div className="flex flex-col md:flex-row gap-3 items-start md:items-center">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setSelectedYear(y => y - 1)} className="p-1.5 rounded hover:bg-muted">
              <ChevronLeft size={16} />
            </button>
      ...
      {loading ? (
        ...
        <Card className="p-12 text-center text-muted-foreground">
          ...
          <p className="text-sm mt-1">Belum ada jurnal pada periode ini.</p>
        </Card>
      ) : (
        <>
              ...
              <tbody className="divide-y divide-border">
                {filtered.map((journal) => (
                  <tr key={journal.id} className="hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-4 py-3 font-medium">{journal.teacher?.name || "-"}</td>
                    <td className="px-4 py-3">
                      <Badge variant="neutral">{journal.class?.name || "-"}</Badge>
                    </td>
                    <td className="px-4 py-3 text-muted-foreground">{journal.subject?.name || "-"}</td>
                    <td className="px-4 py-3 max-w-[200px] truncate">{journal.material}</td>
                    <td className="px-4 py-3 text-center">
                      <Button variant="outline" size="sm" onClick={() => setSelectedJournal(journal)}>
